...
public class DateUtil {
	 ...
	 * 获取指定时间片的整数值
	 */
	public static long getValueByTimeSlice(String timeSliceUnit, long timesliceValue, String mode) {
		if (StringUtils.isBlank(timeSliceUnit) || timesliceValue <= 0) {
			return 0;
		}
		double timeSliceRange;
		if ("y".equals(timeSliceUnit)) {
			timeSliceRange = 365 * 24 * 60 * 60 * 1000;
		} else if ("M".equals(timeSliceUnit)) {
			timeSliceRange = 30 * 24 * 60 * 60 * 1000;
		} else if ("d".equals(timeSliceUnit)) {
			timeSliceRange = 24 * 60 * 60 * 1000;
		} else if ("h".equals(timeSliceUnit)) {
			timeSliceRange = 60 * 60 * 1000;
		} else if ("m".equals(timeSliceUnit)) {
			timeSliceRange = 60 * 1000;
		} else {
			timeSliceRange = 1000;
		}
		double result = timesliceValue / timeSliceRange;
		switch (mode) {
			case "floor": // 向下
